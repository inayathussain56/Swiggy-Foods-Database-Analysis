...
-- Now our tables are ready, now we write queries to answer the business Questions.
...
-- Replace 
-- x with the desired sales threshold. For example, if 
-- x is 5000, the query would be:
...
SELECT o.order_id, o.date, o.amount, f.f_name, od.id
FROM orders o
JOIN order_details od ON o.order_id = od.order_id
JOIN food f ON od.f_id = f.f_id
WHERE o.user_id = 1
  AND o.date BETWEEN '2022-06-01' AND '2022-06-30'
ORDER BY o.date, o.order_id, od.id;

-- (6) Find restaurants with max repeated customers

SELECT r.r_name, COUNT(DISTINCT o.user_id) AS repeated_customers
FROM orders o
JOIN restaurants r ON o.r_id = r.r_id
WHERE o.user_id IN (
    SELECT user_id
...
CustomerFavoriteFood AS (
    SELECT
        user_id,
        f_name,
        order_count,
        ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY order_count DESC) AS row_num
    FROM CustomerFoodCounts
)
SELECT
    user_id,
    f_name AS favorite_food,
    order_count
FROM CustomerFavoriteFood
WHERE row_num = 1;
...
MostLoyalCustomers AS (
    SELECT
        coc.r_id,
        coc.user_id,
        coc.order_count,
        ROW_NUMBER() OVER (PARTITION BY coc.r_id ORDER BY coc.order_count DESC) AS rank
...
SELECT
    ...
    coc.order_count
FROM MostLoyalCustomers coc
JOIN restaurants r ON coc.r_id = r.r_id
...
RankedCustomers AS (
    SELECT
        ...
        coc.user_id,
        coc.order_count,
        ROW_NUMBER() OVER (PARTITION BY coc.r_id ORDER BY coc.order_count DESC) AS row_num
    FROM CustomerOrderCounts coc
)
SELECT
    r.r_name,
